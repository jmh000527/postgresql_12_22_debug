...
static Query *current_hint_query = NULL;
...
get_current_hint_string(ParseState *pstate, Query *query)
{
	...
	const char *outline_hint;

	if (query != current_hint_query)
	{
		current_hint_query = query;
	...
	/* Outline-provided hints override everything else. */
	outline_hint = pg_outline_get_hint(query);
	if (outline_hint)
	{
		if (current_hint_str)
			pfree((void *) current_hint_str);
		oldcontext = MemoryContextSwitchTo(TopMemoryContext);
		current_hint_str = pstrdup(outline_hint);
...
{
	/* Capture outline metadata first. */
	pg_outline_post_parse(pstate, query);
						 ...
						 List *initial_rels)
{
	...
	/*
	 * Switch hint context when the planner starts working on a different
	 * query block (e.g., subquery). This allows outlines to supply per-block
	 * hints.
	 */
	if (root->parse != current_hint_query)
	{
		current_hint_retrieved = false;
		get_current_hint_string(NULL, root->parse);
		if (current_hint_str)
			current_hint_state = create_hintstate(root->parse,
												  pstrdup(current_hint_str));
		else
			current_hint_state = NULL;
	}
